...
public class Main {
    ...
    private static boolean isWithInRange(int x, int y){
        if(1 <= x && 1 <= y && x <= N && y <= N) return true;
        else return false;
    }
    ...
    private static int[][] deepCopy(int[][] copytestCase){
        System.out.println("길이 : " + copytestCase.length + " " + copytestCase[0].length);
        int[][] testCase = new int[copytestCase.length][copytestCase[0].length];

        for(int i = 1; i < copytestCase.length; i++){
    ...
    private static void rotate90(){
        ...
        for(int i = 1; i <= N; i++){
            for(int j = 1; j <= N; j++){
                if(arr[i][j] > 0){
                    int curDistance = getCalculateStreet(j, i) + 1;
    ...
    private static void rightRotate90(int row, int col, int distance){
        int[][] testCase = new int[distance + 2][distance + 2];
        ...
        for(int i = row; i <= row + distance; i++){
            for(int j = col; j <= col + distance; j++){
                ...
                testCase[i - row + 1][j - col + 1] = arr[i][j];
            }
        }
        ...
        for(int i = 1; i <= distance + 1; i++){
            for(int j = 1; j <= distance + 1; j++){
                testCase[i][j] = testCase2[distance - j + 1][i];
            }
        }
        ...
        for(int i = row; i <= row + distance; i++){
            for(int j = col; j <= col + distance; j++){
                arr[i][j] = testCase[i - row + 1][j - col + 1];
